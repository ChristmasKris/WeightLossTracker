...
export const parse = {
	...
	* @returns {int|null} An integer, or null if it is not an integer.
	*/
	int(value) {
		if ((value === null) || (value === undefined) || (typeof value === 'boolean') || (typeof value === 'object')) {
			logError('parse.int(): Wrong variable type.', {
				value,
				type: typeof value
			});
		...
		if (typeof value === 'string') {
			...
			if (!trimmed) {
				logError('parse.int(): Empty string.', {
					value,
					trimmed
				});
			...
			if (!match) {
				logError('parse.int(): Non-integer string format.', {
					value,
					trimmed
				});
		...
		if (!Number.isInteger(number)) {
			logError('parse.int(): Not an integer.', {
				number
			});
		...
		if (!Number.isSafeInteger(number)) {
			logError('parse.int(): Not a safe integer.', {
				number
			});
	...
	* @returns {float|null} A float, or null if it is not a float.
	*/
	float(value) {
		if ((value === null) || (value === undefined) || (typeof value === 'boolean') || (typeof value === 'object')) {
			logError('parse.float(): Wrong variable type.', {
				value,
				type: typeof value
			});
		...
		if (typeof value === 'string') {
			...
			if (!trimmed) {
				logError('parse.float(): Empty string.', {
					value,
					trimmed
				});
			...
			if (trimmed.includes(',')) {
				logError('parse.float(): Non-float string format.', {
					value,
					trimmed
				});
		...
		if (!Number.isFinite(number)) {
			logError('parse.float(): Not finite.', {
				number
			});
	...
	*/
	string(value) {
		...
		if (typeof value === 'string') {
			...
			if (!trimmed) {
				logError('parse.string(): Empty string.', {
					value,
					trimmed
				});
		...
		if (typeof value === 'number') {
			if (!Number.isFinite(value)) {
				logError('parse.string(): Not finite number string.', {
					value
				});
		...
		logError('parse.string(): Could not convert variable to string.', {
			value
		});
	...
	* @returns {boolean|null} A boolean, or null if it is not a valid boolean.
	*/
	boolean(value) {
		if ((value === null) || (value === undefined)) {
			logError('parse.boolean(): Value is null or undefined.', {
				value,
				type: typeof value
			});
		...
		if (typeof value === 'string') {
			...
			logError('parse.boolean(): String is not a valid boolean representation.', {
				value,
				trimmed
			});
		...
		if (typeof value === 'number') {
			...
			logError('parse.boolean(): Number is not a valid boolean representation (must be 0 or 1).', {
				value
			});
		...
		logError('parse.boolean(): Wrong variable type.', {
			value,
			type: typeof value
		});
	...
	/**
	* Convert a value to an integer or return a default value.
	* @param {int} value The integer value.
	* @param {unknown} defaultValue The default value.
	* @returns {int|unknown} An integer if it is valid or the default value.
	*/
	...
	/**
	* Convert a value to a float or return a default value.
	* @param {float} value The float value.
	* @param {unknown} defaultValue The default value.
	* @returns {float|unknown} A float if it is valid or the default value.
	*/
	...
	/**
	* Convert a value to a string or return a default value.
	* @param {string} value The string value.
	* @param {unknown} defaultValue The default value.
	* @returns {string|unknown} A string if it is valid or the default value.
	*/
	...
	/**
	* Convert a value to a boolean or return a default value.
	* @param {boolean|string|number} value The boolean value.
	* @param {unknown} defaultValue The default value.
	* @returns {boolean|unknown} A boolean if it is valid or the default value.
	*/
	booleanOrDefault(value, defaultValue = null) {
